 ...
 *
 * Default colours resolve through `useTheme()` so an icon rendered without an
 * explicit `color` follows the active palette.
 */
...
export function GearIcon({ size = 19, color, strokeWidth = 1.6, ...rest }: IconProps) {
  const { colors } = useTheme();
  const stroke = color ?? colors.iconMuted;
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...rest}>
      <Path d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" stroke={stroke} strokeWidth={strokeWidth} />
      <Path
 ...
 */
export function BackChevronIcon({ size = 16, color, strokeWidth = 1.8, ...rest }: IconProps) {
  const { colors } = useTheme();
...
export function ChevronRightIcon({ size = 14, color, strokeWidth = 1.5, ...rest }: IconProps) {
  const { colors } = useTheme();
...
export function SendIcon({ size = 13, color, strokeWidth = 1.8, ...rest }: IconProps) {
  const { colors } = useTheme();
  const stroke = color ?? colors.canvas;
  return (
    <Svg width={size} height={size} viewBox="0 0 16 16" fill="none" {...rest}>
      <Path
...
export function StartDot({ size = 10, color, ...rest }: IconProps) {
  const { colors } = useTheme();
...
export function EndRing({ size = 10, color, strokeWidth = 2, ...rest }: IconProps) {
  const { colors } = useTheme();
 ...
 */
export function CheckIcon({ size = 15, color, strokeWidth = 1.8, ...rest }: IconProps) {
  const { colors } = useTheme();
  const stroke = color ?? colors.ink;
  return (
    <Svg width={size} height={size} viewBox="0 0 16 16" fill="none" {...rest}>
 ...
 */
export function PlaneIcon({ size = 16, color, strokeWidth = 1.8, ...rest }: IconProps) {
  const { colors } = useTheme();
  const stroke = color ?? colors.ink;
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...rest}>
